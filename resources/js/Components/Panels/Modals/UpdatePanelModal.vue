<template>
  <BaseModalForm
      :isOpen="isOpen"
      title="Edytuj model panelu"
      message="Zaktualizuj dane panelu fotowoltaicznego"
  ...
  >
    <template #icon>
      ...
      >
        <PencilSquareIcon class="h-6 w-6 text-green-600" aria-hidden="true" />
      </div>
    </template>
...
const emiter = defineEmits<{
  (e: "close"): void;
  (e: "confirm", updatedPanel: object): void;
}>();
...
// Watch event to open modal with panel data
onMounted(() => {
  watchEvent("show-update-panel-model-modal", (panel: any) => {
    currentPanel.value = panel;
    isOpen.value = true;

    // Populate form fields
    name.value = panel.name;
    manufacturer.value = panel.manufacturer;
    powerWatt.value = panel.power_watt;
  });
});
...
const onSubmit = handleSubmit(async (values) => {
  ...
  console.log("Updating panel:", values);

  router.put(route("panels.update", { panel: currentPanel.value.id }), values, {
    onSuccess: () => {
