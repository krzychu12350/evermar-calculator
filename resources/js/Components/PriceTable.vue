...
const props = defineProps({
  panelModels: { type: Array, required: true },
  variants: { type: Array, required: true },
})

const selectedModels = ref(props.panelModels.map(m => m.id))
const selectedTypes = ref(["String", "Storage"])
const selectedStorage = ref([5, 10, 15])

const filteredPrices = computed(() =>
    props.variants.map(variant => {
      const panels = variant.panel_prices
          ?.filter(p => selectedModels.value.includes(p.panel_model_id))
          .map(p => ({
            ...p,
      ...
      const editStorage = variant.storage_prices
          ? Object.fromEntries(Object.keys(variant.storage_prices).map(k => [k, false]))
          : {}
